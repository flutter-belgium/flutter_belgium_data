...
Future<void> _downloadProjects(http.Client client, String outputPath) async {
  ...
  final projects = await _fetchJson(client, '$_base/projects/minimized_all.json') as List<dynamic>;
  for (final project in projects) {
    final name = (project as Map<String, dynamic>)['name'] as String;
    final encoded = Uri.encodeComponent(name);
    final info = await _fetchJson(client, '$_base/projects/$encoded/info.json') as Map<String, dynamic>;
    ...
    final screenshots = (images['screenshotUrls'] as List<dynamic>?)?.cast<String>() ?? [];
    if (icon != null) {
    ...
    for (final screenshot in screenshots) {
      await _download(client, screenshot, '$outputPath/projects/$name/${_filename(screenshot)}');
    }
  }
}
...
Future<void> _downloadCompanies(http.Client client, String outputPath) async {
  ...
  final companies = await _fetchJson(client, '$_base/companies/minimized_all.json') as List<dynamic>;
  for (final company in companies) {
    final name = (company as Map<String, dynamic>)['name'] as String;
    final encoded = Uri.encodeComponent(name);
    final info = await _fetchJson(client, '$_base/companies/$encoded/info.json') as Map<String, dynamic>;
    final images = (info['images'] as Map<String, dynamic>?) ?? {};
...
Future<void> _downloadDevelopers(http.Client client, String outputPath) async {
  ...
  final developers = await _fetchJson(client, '$_base/developers/minimized_all.json') as List<dynamic>;
  for (final dev in developers) {
    ...
    if (avatarUrl.isNotEmpty) {
      await _download(client, avatarUrl, '$outputPath/developers/$username/avatar.jpg');
    }
  }
}
